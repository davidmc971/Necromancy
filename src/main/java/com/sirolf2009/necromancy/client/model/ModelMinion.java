...
public class ModelMinion extends ModelBase {
    ...
    public void updateModel(EntityMinion minion, boolean shouldUpdate) {
        ...
        if (shouldUpdate) {
            ...
            if (parts.length > 0 && parts[0] != null && parts[0].length > 0 && parts[0][0] != null && (mob = NecroEntityRegistry.registeredEntities.get(parts[0][0].name)) != null && (head = mob.head) == null) {
                head = mob.head == null ? mob.updateParts(ModelMinion.instance).head : mob.head;
            }
            if (parts.length > 1 && parts[1] != null && parts[1].length > 0 && parts[1][0] != null && (mob = NecroEntityRegistry.registeredEntities.get(parts[1][0].name)) != null && (torso = mob.torso) == null) {
                torso = mob.torso == null ? mob.updateParts(ModelMinion.instance).torso : mob.torso;
            }
            if (parts.length > 2 && parts[2] != null && parts[2].length > 0 && parts[2][0] != null && (mob = NecroEntityRegistry.registeredEntities.get(parts[2][0].name)) != null && (armLeft = mob.armLeft) == null) {
                armLeft = mob.armRight == null ? mob.updateParts(ModelMinion.instance).armRight : mob.armRight;
            }
            if (parts.length > 3 && parts[3] != null && parts[3].length > 0 && parts[3][0] != null && (mob = NecroEntityRegistry.registeredEntities.get(parts[3][0].name)) != null && (armRight = mob.armRight) == null) {
                armRight = mob.armLeft == null ? mob.updateParts(ModelMinion.instance).armLeft : mob.armLeft;
            }
            if (parts.length > 4 && parts[4] != null && parts[4].length > 0 && parts[4][0] != null && (mob = NecroEntityRegistry.registeredEntities.get(parts[4][0].name)) != null && (legs = mob.legs) == null) {
    ...
    BodyPart[][] parts;
    float[] torsoPos = new float[3], armLeftPos = new float[3],
            armRightPos = new float[3], headPos = new float[3];
